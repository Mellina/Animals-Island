...
public class Enemy : MonoBehaviour {
	...
	//Quando um atributo precisa de ser publico para as outras classes o enxergarem mas eu não preciso de mostrar no Hierarchy
	[HideInInspector]
	...
	void Update () {
		if (!isStunned)
		{
			if (Time.time >= _moveTime) {
				...
				//Teste! Prestar atenção se não vai prejudicar outros inimigos
				if (moveSpeed != 0) {
					EnemyMovement();
				}
			} else {
				//If adicionado por Mel, só seto o Moving se o Enemy tiver movimento
				if ((myWaypoints.Length != 0)) {
					//So comentei para paraar de aparecer mensagem de warning
//					_animator.SetBool("Moving", false);
	...
	void EnemyMovement() {
		...
		if ((myWaypoints.Length != 0) && (_moving)) {
			...
			if (Mathf.Abs(_vx) <= 0.05f) {
				...
				// enemy is moving
				//Manter comentado para parar de aparecer mensagem de warning, voltar para o normal depois
	...
	public void Stunned()
	{
		if (!isStunned) 
		{
			...
			//Muda o Layer do inimigo para outro para evitar a colisão com o player quando já estiver morrendo
			this.stunnedCheck.layer = _stunnedLayer;
